    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 glass-card p-4 sm:p-5 bg-white border-slate-200/80 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base sm:text-lg font-extrabold text-slate-900">Customer CRM & 360° Profiles</h2>
        ...
        <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
          <div className="relative flex-1 sm:w-72">
            ...
            <input
              ...
              className="input-field pl-9 text-xs py-1.5 sm:py-2"
            />
          </div>
          <button onClick={() => setShowAddModal(true)} className="btn-gold text-xs font-bold px-3 sm:px-4 py-2 shrink-0 shadow-sm">
            <Plus className="w-4 h-4" /> New Customer
          </button>
        </div>
      </div>

      {/* Customer Directory Table with responsive horizontal touch scroll */}
          ...
          <table className="w-full text-left text-xs text-slate-700 min-w-[650px]">
            <thead className="bg-slate-50 text-slate-600 uppercase font-bold text-[10px] tracking-wider border-b border-slate-200">
              <tr>
                ...
                <th className="p-3 sm:p-4">Contact Info</th>
                <th className="p-3 sm:p-4">Wallet Balance</th>
                <th className="p-3 sm:p-4">Loyalty Points</th>
                <th className="p-3 sm:p-4">Total Spent</th>
              ...
              {customers.length === 0 ? (
                ...
                customers.map((c) => (
                  ...
                  >
                    ...
                    <td className="p-3 sm:p-4 space-y-0.5">
                    ...
                    <td className="p-3 sm:p-4 font-bold text-emerald-700">₹{c.walletBalance || 0}</td>
                    <td className="p-3 sm:p-4 font-bold text-brand-700">{c.loyaltyPoints || 0} pts</td>
                    <td className="p-3 sm:p-4 font-bold text-slate-900">₹{c.totalSpent || 0}</td>
      ...
      {selectedCustomer && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex justify-end">
          <div className="w-full sm:max-w-xl bg-white border-l border-slate-200 h-full overflow-y-auto p-4 sm:p-6 space-y-5 sm:space-y-6 shadow-2xl animate-in slide-in-from-right duration-200">
            <div className="flex items-center justify-between pb-3 sm:pb-4 border-b border-slate-200">
              <div>
                <h3 className="text-base sm:text-lg font-extrabold text-slate-900">{selectedCustomer.fullName}</h3>
                <p className="text-xs text-slate-500">{selectedCustomer.phone} • {selectedCustomer.email || 'No email'}</p>
              </div>
              <button onClick={() => setSelectedCustomer(null)} className="p-2 text-slate-400 hover:text-slate-700 rounded-lg hover:bg-slate-100">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="p-3.5 sm:p-4 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-between shadow-sm">
                <div>
                  <div className="text-[11px] text-slate-500">Prepaid Wallet</div>
                  <div className="text-lg sm:text-xl font-bold text-emerald-700 mt-0.5 sm:mt-1">₹{selectedCustomer.walletBalance || 0}</div>
                </div>
                <button onClick={() => setShowTopupModal(true)} className="btn-secondary py-1.5 px-3 text-xs font-semibold">
                  + Topup
                </button>
              </div>

              <div className="p-3.5 sm:p-4 rounded-xl bg-slate-50 border border-slate-200 shadow-sm">
                <div className="text-[11px] text-slate-500">Loyalty Rewards</div>
                <div className="text-lg sm:text-xl font-bold text-brand-700 mt-0.5 sm:mt-1">{selectedCustomer.loyaltyPoints || 0} pts</div>
              </div>
            </div>

            {/* Hair & Skin Technical Profile */}
            <div className="p-3.5 sm:p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-2 text-xs shadow-sm">
      ...
      {showTopupModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4">
          <div className="max-w-sm w-full glass-card p-5 sm:p-6 bg-white border-slate-200 shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowTopupModal(false)} className="btn-secondary flex-1 text-xs py-2 font-semibold">
                Cancel
              </button>
