    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 glass-card p-4 sm:p-5 bg-white border-slate-200/80 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base sm:text-lg font-extrabold text-slate-900">Live Salon Floor & Waiting Lounge</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div className="glass-card p-4 sm:p-5 bg-white border-slate-200/80 shadow-sm">
          ...
          <div className="space-y-2.5 sm:space-y-3">
            {loungeGuests.length === 0 ? (
              <div className="text-center py-8 sm:py-10 text-slate-400 text-xs">No guests waiting in lounge</div>
            ) : (
              loungeGuests.map((guest) => (
                <div key={guest._id || guest.id} className="p-3.5 sm:p-4 rounded-xl bg-slate-50 border border-slate-200 flex flex-col justify-between gap-3 shadow-xs">
                  ...
                  <div className="flex flex-wrap xs:flex-nowrap gap-2 pt-2 border-t border-slate-200">
                    <button
                      onClick={() => updateStatus(guest._id || guest.id, 'IN_SERVICE')}
                      className="btn-gold flex-1 py-1.5 px-3 text-xs font-bold"
                    >
                      <Play className="w-3.5 h-3.5" /> Start Service (Seat)
                    </button>
                    <button
                      onClick={() => updateStatus(guest._id || guest.id, 'NO_SHOW')}
        ...
        <div className="glass-card p-4 sm:p-5 bg-white border-slate-200/80 shadow-sm">
          ...
          <div className="space-y-2.5 sm:space-y-3">
            {inServiceGuests.length === 0 ? (
              <div className="text-center py-8 sm:py-10 text-slate-400 text-xs">No clients currently in chairs</div>
            ) : (
              inServiceGuests.map((guest) => (
                <div key={guest._id || guest.id} className="p-3.5 sm:p-4 rounded-xl bg-slate-50 border border-emerald-200 flex flex-col justify-between gap-3 shadow-xs">
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="font-bold text-sm text-slate-900">{guest.customerName}</div>
                  ...
                  <div className="flex flex-wrap xs:flex-nowrap gap-2 pt-2 border-t border-slate-200">
                    <button
                      ...
                      className="btn-gold flex-1 py-1.5 px-3 text-xs font-bold"
                    >
                      <CheckCircle2 className="w-3.5 h-3.5" /> Bill at POS
                    </button>
                    <button
                      onClick={() => updateStatus(guest._id || guest.id, 'COMPLETED')}
                      className="btn-secondary py-1.5 px-3 text-xs font-semibold hover:bg-emerald-50 hover:text-emerald-700 shrink-0"
